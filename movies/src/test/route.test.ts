...
import * as authentication from '../middleware/user.middleware';
import app from '../app';
...
describe('Endpoints', () => {
    ...
    beforeEach('login', async function () {
     return  sinon.stub(authentication, "authUser").callsFake((req: Request, res: Response, next: NextFunction) => { next() });
    ...
    describe('Login', function () {
        it('Should return user token', async function () {
            ...
            const { status, body } = await chai.request(app).post('/api/v1/login').send({ username: username, password: password })
